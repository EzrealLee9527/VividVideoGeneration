...
def main(args):
    ...
    *_, func_args = inspect.getargvalues(inspect.currentframe())
    ...
    for idx, (source_image, test_video, random_seed, size, step) in tqdm(
        ...
        # img_for_face_det = torch.tensor(np.array(source_image)).to(device, torch.uint8).unsqueeze(0).permute(0, 3, 1, 2)
        ...
        #     # face_image_pil.save('face_test.png')
        ...
        if args.rank == 0:
            ...
            save_videos_grid(
                samples_per_video, f"{savedir}/videos/{source_name}_{video_name}.gif", save_every_image=save_every_image)
            # save_videos_grid(
