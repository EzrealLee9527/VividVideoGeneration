...
def main(args):
    ...
    video_metafile = args.infile
    outdir = args.outdir
    tar_split_size = 100
    
    if video_metafile.endswith('.json'):
        video_meta = load_json(video_metafile)
    elif megfile.smart_isdir(video_metafile):
        video_meta = dict()
        for jsonf in megfile.smart_glob(os.path.join(video_metafile,'*.json')):
            video_meta.update(
                **load_json(jsonf)
            )
    ...
    video_meta = OrderedDict(sorted(video_meta.items()))
    video_paths = list(video_meta.keys())
    use_tar_member = is_tarmember_path(video_paths[0])
    if use_tar_member:
        tar_path_prefixs = list(set([ p.split(".tar/")[0] for p in video_paths]))
        print(f'Tar files : {len(tar_path_prefixs)}')
        video_paths2job_idx = {
            p:tar_path_prefixs.index(p.split(".tar/")[0])
            for _,p in enumerate(video_paths)
        }
    ...
    for video_idx,(video_file, clips_metas) in tqdm(enumerate(video_meta.items())):
        ...
        job_idx = video_paths2job_idx[video_file]
