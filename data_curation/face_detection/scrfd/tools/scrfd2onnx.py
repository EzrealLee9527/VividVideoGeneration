...
def pytorch2onnx(config_path,
    ...
    model.eval()
    model.forward = model.forward_for_dump
    
    tensor_data[0] = tensor_data[0].detach()  # Detach if the input tensor was part of another computation
    tensor_data[0].requires_grad_(False)    # Alternatively, make sure requires_grad is False

    for param in model.parameters():
        param.requires_grad = False
    def model_forward(*args):
        with torch.no_grad():
            return model.forward_for_dump(*args)
    script_model = torch.jit.trace(
        model_forward,tensor_data,optimize=False
    )
    script_model.save(ori_output_file.replace('onnx','pt'))
...
if __name__ == '__main__':
    ...
    if not args.input_img:
        import cv2
        cv2.imwrite( osp.join( osp.dirname(__file__), 't1.jpg'),
            np.random.randint(0,255,size = (640,640,3)).astype('uint8'),
            
        )
        args.input_img = osp.join(
            osp.dirname(__file__), 't1.jpg')
