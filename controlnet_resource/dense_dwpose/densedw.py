...
class DenseDWposePredictor:
    def __init__(self, device, resolution = [512, 512]) -> None:
        ...
        self.dwpose_model = DWposeDetector(
            ...
            pose_ckpt=pose_ckpt,
            device=device)
        self.dense_model = DensePosePredictor(device = device if isinstance(device, int) or isinstance(device, str) else device.index)

    def __call__(self, img):
        dense_frame = self.dense_model(img, convert_rgb=False)
        dw_frame = self.dwpose_model(img, output_type='np', image_resolution=self.resolution[0])
