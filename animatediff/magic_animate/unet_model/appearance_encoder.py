...
class AppearanceEncoderModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin):
    ...
    def from_unet(
        ...
        """
        switch_2d_down_block_types = unet.config.down_block_types
        for i in range(len(switch_2d_down_block_types)):
            switch_2d_down_block_types[i] = switch_2d_down_block_types[i].replace('3D', '2D')
        
        switch_2d_mid_block_type = unet.config.mid_block_type.replace('3D', '2D')
        
        switch_2d_up_block_types = unet.config.up_block_types
        for i in range(len(switch_2d_up_block_types)):
            switch_2d_up_block_types[i] = switch_2d_up_block_types[i].replace('3D', '2D')
        ...
        controlnet = cls(
            ...
            down_block_types=switch_2d_down_block_types,
            mid_block_type=switch_2d_mid_block_type,
            up_block_types=switch_2d_up_block_types,
        ...
        if load_weights_from_unet:
            ...
            m, u = controlnet.up_blocks.load_state_dict(unet.up_blocks.state_dict(), strict=False)
