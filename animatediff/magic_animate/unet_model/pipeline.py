...
from animatediff.magic_animate.unet_model.unet_controlnet import UNet3DConditionModel
from animatediff.magic_animate.unet_model.controlnet import ControlNetModel
from animatediff.magic_animate.unet_model.mutual_self_attention import ReferenceAttentionControl
...
class AnimationPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        for i, t in tqdm(enumerate(timesteps), total=len(timesteps), disable=(rank != 0)):
            ...
            if appearance_encoder is not None:
                # print('ref_image_latents shape is', ref_image_latents.shape)
                # print('text_embeddings shape is', text_embeddings.shape)
                # print('t is', t)
                # appearance_encoder(
                #     ref_image_latents.repeat(16, 1, 1, 1).repeat(
                #         context_batch_size * (2 if do_classifier_free_guidance else 1), 1, 1, 1),
                #     t,
                #     encoder_hidden_states=text_embeddings.repeat(16, 1, 1),
                #     return_dict=False,
                # )
                appearance_encoder(
            ...
            # print('context_queue for one is', context_queue)
            ...
            for i in range(num_context_batches):
                ...
                # print('controlnet_latent_input shape is', controlnet_latent_input.shape)
                # print('t is', t)
                ...
                # print('controlnet_cond shape is', controlnet_cond.shape)
                # print('conditioning_scale is', controlnet_conditioning_scale)
                encoder_hidden_states=torch.cat(
                        [controlnet_text_embeddings_c[c] for c in context])
                controlnet_cond=torch.cat(
                        [controlnet_cond_images[c] for c in context])
                ...
                # print('infer context is', context)
    ...
    def train(
        ...
        down_block_res_samples, mid_block_res_sample = self.controlnet(
            controlnet_latent_input[:, :4, ...],
            t.repeat(video_length),
        ...
        # for x in down_block_res_samples:
        #     print('train down_block_res_samples shape is', x.shape)
        # print('train mid_block_res_sample shape is', mid_block_res_sample.shape)
        # print('train batchsize is', batch_size)
        # print('train context_frames is', context_frames)
        # print('train context_batch_size is', context_batch_size)
