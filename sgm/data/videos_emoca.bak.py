...
class S3VideosDataset(Dataset):   
            ...
            for worker_data_dir in smart_glob(data_dir + '/*'):
                ...
                for video_data_dir in video_data_dirs:
                    ...
                    if num_frames > 0:
                        ...
                        if DEBUG and len(videos_info_list) > 16:
    ...
    def get_labels(self, video_info_dir, num_frames):
        ...
        h = crop_y2 -crop_y1
        w_delta = max(0, (h - (x_max-x_min)) / 2 + (new_width-new_height)//2)
        ...
        frames_shape_detail_imgs = frames_shape_detail_imgs / 255.0
        ...
        frames_shape_imgs = frames_shape_imgs / 255.0
        ...
        # frames_imgs = rearrange(frames_imgs, "b t c h w -> (b t) c h w", t=self.video_length)
        ...
        sample.update(value_dict)

        sample['num_video_frames'] = self.video_length
        sample['image_only_indicator'] = torch.zeros(1, self.video_length)
...
if __name__ == '__main__':
    ...
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=3, num_workers=1, collate_fn=collate_fn)
    
    for idx, batch in enumerate(dataloader):
        
        print('gen',batch["pixel_values"].shape, batch["pixel_values"].shape)
        ...
        if is_image:
            ...
            if batch["shape_detail_imgs"].ndim == 4:
                batch["shape_detail_imgs"] = rearrange(batch["shape_detail_imgs"], "(b t) c h w -> b t c h w", t=video_length)
                batch["shape_imgs"] = rearrange(batch["shape_imgs"], "(b t) c h w -> b t c h w", t=video_length)
                batch["pixel_values"] = rearrange(batch["pixel_values"], "(b t) c h w -> b t c h w", t=video_length)
